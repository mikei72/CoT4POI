...
PROMPT_TEMPLATE = (
    "<s>[INST] <<SYS>>\n"
    "You are a Next-POI recommendation assistant. Your goal is to predict the user's next visit by reasoning over their history and preference, while strictly adhering to spatio-temporal logic.\n"
...
"""PROMPT_TEMPLATE = (
    "<s>[INST] <<SYS>>\n"
    "You are a Next-POI recommendation assistant.\n"
    "User Preference: {preference}\n"
    "<</SYS>>\n\n"
    "{history}\n\n"
    "### Auxiliary Prediction Signals:\n"
...
def main(args):
    ...
    data_path = f'datasets/{args.dataset_name}/'
    full_test_path = os.path.join(data_path, args.test_file)

    print(f"Loading test data from: {full_test_path}")
    ...
    for index, item in tqdm(enumerate(test_data), desc="Processing lines", total=len(test_data)):
        gt_text = item.get('target_answer', '')

        match = re.search(r"(.*POI id\s*)(\d+)", gt_text)
        if not match:
            print(f"Ground Truth format not match: {gt_text}")
            continue
        ...
        prompt = tokenizer(final_input_text, return_tensors="pt").to(device)

        if prompt.input_ids.shape[1] >= 32768:
            print(f"Warning: Prompt too long ({prompt.input_ids.shape[1]}), skipping index {index}")
            continue
        ...
        prediction = tokenizer.decode(outputs[:, prompt.input_ids.shape[1]:][0], skip_special_tokens=True)
        prediction = re.sub(r'[^0-9]', '', prediction)

        print(prediction, ' ', gt_text)
    ...
    acc_1 = correct_predictions_1 / len(test_data)
    print(f'ACC@1:{acc_1}')
    ...
    output_file = parent_dir / "evaluation_results.json"

    result_data = {
        "ACC@1": acc_1,
        "correct_index": correct_list
    }

    with open(output_file, 'w', encoding='utf-8') as f:
        json.dump(result_data, f, ensure_ascii=False, indent=4)

    print(f"评估结果已成功保存至: {output_file}")
